...
class EdgePoseDatasetV6(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_kpts = keypoints_2d.copy()

        # Generate target heatmaps
        target_heatmaps = generate_heatmaps(keypoints_2d, self.output_res, sigma=4) # adjust to sigma=3

        target_heatmaps = target_heatmaps.float()
        gt_kpts = torch.from_numpy(gt_kpts).float()

        return input_tensor, target_heatmaps, gt_kpts